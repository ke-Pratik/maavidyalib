...
import { getAllFeeStatus, generateAllFees, getStudentsWithNoConfig } from "../services/api";
...
function AllFeeStatus() {
  ...
  const handleGenerateAll = async () => {
    ...
    try {
      ...
      toast.success(`Generated: ${res.data.generated} | Skipped: ${res.data.skipped} | No Config: ${res.data.noConfig}`);
  ...
  const handleCheckNoConfig = async () => {
    ...
    try {
      ...
      if (res.data.count === 0) toast.success("All students have fee config locked! ✅");
    ...
    <div>
      ...
      <div className="form-section col-lg-10 mb-4">
        <form onSubmit={handleFetch} className="d-flex gap-3 align-items-end flex-wrap">
          ...
          <div>
            ...
            <input type="number" className="form-control" style={{ width: "100px" }} value={year}
              onChange={(e) => { setYear(e.target.value); setGenResult(null); setNoConfigData(null); }} />
          </div>
          <button type="submit" className="btn btn-primary" disabled={loading}>
            {loading ? "Loading..." : "🔍 Fetch"}
          </button>
          <button type="button" className="btn btn-warning" onClick={handleGenerateAll} disabled={genLoading}
            title="Creates PENDING records for all active students who don't have one yet">
            {genLoading ? <><span className="spinner-border spinner-border-sm me-2" />Generating...</> : "⚡ Generate All Fees"}
          </button>
          {/* ENHANCEMENT #4: Check no-config students */}
          <button type="button" className="btn btn-outline-danger" onClick={handleCheckNoConfig}
            disabled={noConfigLoading} title="Shows students who have never had fee locked">
      ...
      {genResult && (
        <div className="alert alert-warning col-lg-10 mb-4">
          <h6 className="fw-bold mb-2">⚡ Fee Generation — {MONTH_NAMES[genResult.month - 1]} {genResult.year}</h6>
          <div className="d-flex gap-4 flex-wrap mb-2">
            <span><strong>Total Active:</strong> {genResult.totalActiveStudents}</span>
            <span><strong className="text-success">✅ Generated:</strong> {genResult.generated}</span>
            <span><strong className="text-secondary">⏭ Skipped:</strong> {genResult.skipped}</span>
            <span><strong className="text-danger">⚠️ No Config:</strong> {genResult.noConfig}</span>
          </div>
          {genResult.noConfig > 0 && (
            <div>
              <small className="text-danger fw-bold">Lock fee for these students from Fee Calculate page:</small>
              <div className="mt-1 d-flex gap-2 flex-wrap">
                {genResult.noConfigRegNos.map((rn) => <span key={rn} className="badge bg-danger">Reg #{rn}</span>)}
              </div>
            </div>
          )}
        </div>
      ...
      {noConfigData && (
        <div className={`alert col-lg-10 mb-4 ${noConfigData.count > 0 ? "alert-danger" : "alert-success"}`}>
            ...
            <div className="table-responsive">
              <table className="table table-sm mb-0">
                <thead className="table-dark">
                  <tr><th>Reg No</th><th>Name</th><th>Mobile</th><th>Time Slot</th><th>Admission Date</th></tr>
                </thead>
                <tbody>
          ...
          <button className="btn btn-sm btn-outline-secondary mt-2" onClick={() => setNoConfigData(null)}>✕ Dismiss</button>
        </div>
      )}
        ...
        <div>
          ...
          {filtered.length === 0 ? (
            ...
            <div className="table-responsive">
              <table className="table table-custom table-hover">
                <thead>
                  <tr><th>#</th><th>RegNo</th><th>Name</th><th>Slot</th>
                    <th>Fee</th><th>Paid</th><th>Balance</th><th>Status</th><th>Mode</th><th>Receipt</th></tr>
                </thead>
                <tbody>
                  {filtered.map((s, i) => (
                    // ENHANCEMENT #3: NO_RECORD rows highlighted red
                    <tr key={i} className={s.paymentStatus === "NO_RECORD" ? "table-danger" : ""}>
                      <td>{i + 1}</td>
                      <td className="fw-bold">{s.regNo}</td>
                      <td>{s.studentName}</td>
                      <td>{s.timeSlot || "—"}</td>
          ...
          {data.noRecordCount > 0 && (
            <div className="alert alert-danger mt-3 py-2">
              <small>
                ⚠️ <strong>{data.noRecordCount} students</strong> have no fee record for {MONTH_NAMES[month - 1]} {year}.
                Click <strong>⚡ Generate All Fees</strong> to create PENDING records for all at once.
              </small>
            </div>
          )}
        </div>
